...
var headers = 'Mozilla/5.0 (Windows NT 10.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.101 Safari/537.36';
...
var httpHelper = {
     ...
     */
    get: function (url, timeout, callback, encoding, headers) {

        var options = require('url').parse(url);
        options.method = 'GET';
        if(!headers){
            options.headers={};
            options.headers["Accept"]="ext/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8";
            ...
            //options.headers["Accept-Charset"]="gb2312";
    ...
    getLocal: function (url,encoding='UTF-8',timeout=30000,headers) {
        return new global.Promise((resolve, reject)=>{
            this.get(url, timeout, (err,data)=>{
                if(!err){
                    //console.log(data);
                    resolve(data);
                }else{
    ...
    postLocal: function (url, data, encoding='UFT-8') {
        return new global.Promise((resolve, reject)=>{
